...
| Working draft | Postgres kv (`draft`) | chat | this week |
...
above the model's judgement and below the hard-coded
...
## 3. Working draft — this week (chat)

The current plan-in-progress, written entirely by chat iteration — nightly
housekeeping never touches it; pushed to Garmin only on explicit approve. Days
approved in chat are recorded `source='chat'`.
...
`suggestions` + `outcomes`: each pushed session is recorded (by chat, on push);
the nightly job reconciles the day's Garmin actuals against it (adherence).
